get_categorical_field_labels <- function(metadata) {
  ...
  #
  categorical_fields <- metadata %>% dplyr::filter(field_type %in% c("checkbox", "radio", "yesno")) %>% dplyr::select(c(field_name, field_label, form_name, select_choices_or_calculations))
